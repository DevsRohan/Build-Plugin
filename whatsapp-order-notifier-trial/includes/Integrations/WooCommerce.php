<?php
namespace WON_Trial\Integrations;

use WON_Trial\Notifications\Manager;
...
class WooCommerce {
    ...
    public function track_cart() {
        ...
        $cart_model = new \WON_Trial\Database\AbandonedCart();
    ...
    public function cart_recovered( $order_id ) {
        ...
        $session_id = WC()->session->get_customer_id();
        if ( $session_id ) {
            ( new \WON_Trial\Database\AbandonedCart() )->mark_recovered( $session_id );
        }
    }
}
