...
class Processor {
    ...
    public function process() {
        ...
        try {
            $queue = new \WON_Trial\Database\MessageQueue();
            $log   = new \WON_Trial\Database\NotificationLog();
            $api   = new \WON_Trial\Api\HFWhatsApp();
    ...
    public function cleanup() {
        ...
        ( new \WON_Trial\Database\NotificationLog() )->cleanup( $days );
        ( new \WON_Trial\Database\MessageQueue() )->cleanup( 7 );
        ( new \WON_Trial\Database\AbandonedCart() )->cleanup( $days );
    }
}
